...
«Художка Журнал» объединяет локальный Android-журнал, начальный server workspace
и backend на Django/DRF/PostgreSQL.

Режим `Local legacy` остаётся однопользовательским: десять типов сущностей
хранятся в Room/SQLite и не смешиваются с серверными данными. Отдельный режим
`Server workspace` уже поддерживает JWT-вход, зашифрованное хранение refresh
token, обновление access token, восстановление сессии и явный выбор доступной
школы.
...
| --- | --- | --- |
| **Рабочее пространство** | `ServerWorkspaceScreen` | Раздельные local/server режимы, JWT-вход, восстановление сессии и выбор школы |
...
- ✅ Созданы Retrofit/Moshi API client и единая типизированная модель сетевых
  ошибок.
...
По умолчанию `Server workspace` обращается к `http://10.0.2.2:8000/` —
backend на хосте Android-эмулятора. Для другого HTTPS-окружения URL задаётся во
время сборки и обязан заканчиваться символом `/`:

```bash
./gradlew assembleDebug -PARTJOURNAL_API_BASE_URL=https://api.example.test/
```

Незашифрованный HTTP разрешён только для development-хостов `10.0.2.2` и
`localhost`.
