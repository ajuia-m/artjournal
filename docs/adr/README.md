...
|---|---|---|---|
| [0001](0001-server-backed-architecture.md) | Accepted | Backend-фундамент, модель, импорт, JWT, роли и journal API реализованы; Android не подключён | Перейти от локальной Room-базы к backend на Django/DRF и PostgreSQL. |
| [0002](0002-android-server-integration.md) | Accepted | Data boundary и `Local legacy` repository реализованы; server workspace, JWT-клиент и миграция ожидаются | Подключать Android через явную data boundary, server-authoritative режим и контролируемую миграцию legacy-данных. |
