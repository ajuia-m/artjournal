...
Этот документ описывает фактическую локальную legacy-модель Android-приложения,
которая продолжает работать отдельно от уже реализованного backend. Он нужен
как контрольная точка для API, нормализации схемы и переноса пользовательских
данных. Это не целевая схема PostgreSQL.
...
| Безопасная миграция схемы | Явные migration paths не добавлены, а destructive fallback отключён; неподдерживаемое обновление версии завершится ошибкой открытия базы вместо автоматического удаления данных. |
