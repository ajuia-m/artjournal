...
JWT, роли и API управления членствами реализованы отдельно. Защищённый HTTP
endpoint импорта и Celery пока не входят в этот этап. Backend уже реализует
команды и change feed для `StudentLessonState`, но Android outbox/worker и
остальные типы синхронизации отсутствуют. Будущий авторизованный import
endpoint должен вызывать тот же импорт-сервис, не дублируя правила.
