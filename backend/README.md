...
назначениями преподавателей, а также основной журнал занятий. API прогресса по
зачётным темам, авторизация импорта и подключение Android пока не входят в этот
этап. Импорт по-прежнему доступен только через management-команду.
