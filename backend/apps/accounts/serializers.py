...
class CurrentUserTeachingAssignmentSerializer(serializers.ModelSerializer):
...
class CurrentUserMembershipSerializer(serializers.ModelSerializer):
    ...
    teaching_assignments = CurrentUserTeachingAssignmentSerializer(
        many=True,
        read_only=True,
    )
...
class CurrentUserSerializer(serializers.ModelSerializer):
    ...
    @extend_schema_field(CurrentUserMembershipSerializer(many=True))
    def get_memberships(self, user: User) -> list[dict[str, object]]:
        memberships = getattr(user, "active_school_memberships", ())
        return CurrentUserMembershipSerializer(memberships, many=True).data
